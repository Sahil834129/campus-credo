...
export function getStudentAge(dateOfBirth, isMonth = false) {
  ...
  return baseDate.diff(
    moment(dateOfBirth, getDefaultDateFormat()),
    isMonth ? "months" : "years"
  );
}
...
export const userCanNotApprove = (isApproveY) => {
  ...
  if (modulePermissions !== null) {
    ...
    flag = modulePermissions.find((val) => {
      let isApproveYValid = isApproveY
        ? val.permissionType.indexOf("APPROVE-Y") !== -1
        : val.canApprove;
