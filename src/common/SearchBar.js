...
const SearchBar = () => {
  ...
  const selectedLocation =
    isLoggedIn() && !isEmpty(getLocalData("selectedLocation"))
  ...
  const getSchoolData = async () => {
    ...
    try {
      ...
      setSearchItems(
        response.data.map((school, index) => ({
  ...
  const LocationDropDownToggle = React.forwardRef(
      ...
      <Col className="navbar-header">
        <Container className="header-container">
          <div className="header-item brand-logo">
            <Link to="/" onClick={(e) => gotoHome(e, navigate)}>
          ...
          <div className="header-item search-region-pane">
                ...
                <Dropdown.Menu as={LocationDropDownMenu}>
                  {cities.map((city, index) => {
                    return (
                      <Dropdown.Item
                        key={"city_" + index}
                        eventKey={"city_e_" + index}
                        onClick={(e) => handleSelectCity(city)}
                      >
                        {city ? city : ""}
                      </Dropdown.Item>
                    );
            ...
            <div className="search-wrapper">
              <div
                ...
                style={{ width: 500, marginTop: 4, marginBottom: 4 }}
              >
