...
const VerifyPhone = () => {
  ...
  const verifyOTP = (formData) => {
    ...
    RESTClient.post(RestEndPoint.VERIFY_PHONE, reqPayload)
      .then((response) => {
        setSubmitting(false);
        console.log(queryParams.get("redirectUrl"));
        navigate(
          `${
            queryParams.get("redirectUrl")
              ? atob(queryParams.get("redirectUrl"))
              : "/?login=true"
          }`
        );
      })
        ...
        <div className="signup-col">
          ...
          <div className="form-wrapper verify-phone">
            <div className="form-title">
              <h2>Verify OTP</h2>
              <h4>
                Verify your mobile number with the OTP sent to you via SMS.
              </h4>
            </div>
            <div className="form-container">
              <Formik
                initialValues={{ phone: { phone }, otp: "" }}
