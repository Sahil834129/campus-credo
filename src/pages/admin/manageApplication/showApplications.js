  ...
  const columns = [
      ...
      Cell: (e) => {
        return (
          ...
          >
            <span
              style={{ color: "#41285F" }}
            >{`${e.row.original?.firstName} ${e.row.original?.lastName}`}</span>
          </a>
        );
      },
    },
    ...
    },
    {
      ...
      Cell: (e) => {
        ...
        return (
          <span>
            {marksUnit === "GRADES"
              ? e.row.original.grade
              : e.row.original.obtainMarks}
          </span>
        );
      },
    },
    {
      ...
      Cell: (e) => {
        return (
          <span>
            {moment(e.row.original.submissionDate).format("DD/MM/YYYY")}
          </span>
        );
      },
    },
    {
      ...
      Cell: (e) => {
        ...
        if (e.row.original.anyMedicalDetails) notes.push("medicalConditions");
        if (e.row.original.anyExtracurriculars) notes.push("extracurricular");
        return (
              ...
              notes.map((note, index) => {
                return (
                  <span
                    ...
                    key={"notes_" + index}
                    title={
                      note === "backgroundCheckNegative"
                        ? "Has background history"
                        : note === "medicalConditions"
                        ? "Has medical condition/disability"
                        : note === "extracurricular"
                        ? "Participated state/national/international extracurricular"
                        : ""
                    }
                  >
                    {/* {note.substring(0, 1)} */}
    ...
    },
    {
      ...
      Cell: (e) => {
        ...
        return (
          <span className={getClassName(applicationStatus)}>
            {getStatusLabelForSchool(applicationStatus)}
          </span>
        );
      },
    },
    {
      ...
      Cell: (e) => {
        ...
        if (STATE_TRANSITION[applicationStatus.toUpperCase()]) {
          stateTransiton = STATE_TRANSITION[
            ...
            if (
              ...
              } else return val !== SCHOOL_APPLICATION_STATUS.AT_PI;
            } else {
        ...
        return (
          <>
            {stateTransiton && stateTransiton.length > 0 && (
              <Dropdown>
                <Dropdown.Toggle
                  as={CustomToggle}
        ...
        <div className="btn-wrapper">
          <Button
            ...
            disabled={
              !isWritePermission ||
              (selectedRows &&
                Object.keys(selectedRows).length !== rowsData.length)
            }
            onClick={() => {
              handleBulkStatusUpdate(
