    ...
    <>
      ...
      >
        ...
        <ListGroup className="info-list-group">
          <ListGroup.Item>
            ...
            {school.monthlyFeeMin && school.monthlyFeeMax ? (
              <div className="right fee-wrap">
                <span className="fee-to">₹{school.monthlyFeeMin}</span> to{" "}
                <span className="fee-from">₹{school.monthlyFeeMax}</span>
              </div>
            ) : (
              ...
              {school.admissionInfo != null ? (
                <>
                  <span className="session-title">
                    {school.admissionInfo.admissionStatus} for{" "}
                    {school.admissionInfo.admissionSession}{" "}
                  </span>
                  <InfoDropDown
                    header={
                      school.admissionInfo.admissionStatus +
                      " for " +
                      school.admissionInfo.admissionSession
                    }
                    options={school.admissionInfo.admissionOpenForClasses.split(
                      ","
                    )}
                  />
                </>
          ...
          <ListGroup.Item>
            ...
            <div className="right seats">
              {school.admissionInfo
                ? school.admissionInfo.seatsAvailable
                : "NA"}
            </div>
          </ListGroup.Item>
        </ListGroup>
        <Card.Body className="button-wrap">
          <Card.Link
            onClick={(e) => {
              handleViewDetails(school.schoolId, school.schoolName);
              e.stopPropagation();
            }}
            className="view"
          >
          ...
          {school?.partner && (
            <Card.Link
              ...
              onClick={(e) => {
                handleAddToCart(
                  school.schoolId,
                  school.admissionInfo ? true : false
                );
                e.stopPropagation();
              }}
            >
        ...
        <Col className="salient-features">
          <Row className="partner-wrap">
            {school?.partner && (
              <div>
                <div className="partner-item icon">
                  <i className="icons partner-icon"></i>
                </div>
                <div className="partner-item lbl">
                  <label>Application Partner</label>
                </div>
              </div>
            )}
          </Row>
          <ListGroup className="feature-list-group allfeatures-wrap">
            {school.facilities.map((item, index) => {
              let fIcon = PageContent.FACILITY_ICON_MAP.hasOwnProperty(
                ...
                ? PageContent.FACILITY_ICON_MAP[item.facilityName]
                : null;
              if (index < 4) {
                return (
                  <ListGroup.Item key={item.facilityMasterId}>
                    <i
              ...
              <ListGroup.Item>
                {school.facilities.length > 4 ? (
                  <>
                    ...
                    <InfoDropDown
                      icon={school.facilities.length - 4}
                      options={school.facilities
                        .filter((it, index) => {
                          return index > 3;
                        })
                        .map((it) => it.facilityName)}
                    />
                  </>
                ) : (
                  ""
      ...
      <ApplyToSchoolDialog
        show={showApplyToSchoolDialog}
        schoolId={selectedSchoolToApply}
        handleClose={handleAddToCartDialogClose}
      />
      <AlertDialog
